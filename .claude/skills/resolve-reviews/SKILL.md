...
1. **`AGENTS.md`** (primary, if it exists): check `test -f AGENTS.md` first. If present, its rules take priority over everything else.
2. **Kotlin/Spring Boot best practices** (fallback): apply general Kotlin official guidance and Spring Boot recommendations when `AGENTS.md` is absent or silent on the topic.
...
Always cite a specific source in the rationale (e.g. `AGENTS.md §Logging Style` if it exists, or `Kotlin: prefer val over var` as a general-practice fallback).
...
3. If the changes have not been committed yet, commit them (see `.claude/skills/git-commit/SKILL.md` for the commit format)
...
| 2 | bob | Bar.kt:34 | ❌ INVALID | Kotlin: no dynamic data in exception message | Skipped |
