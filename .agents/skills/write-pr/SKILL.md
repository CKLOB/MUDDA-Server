...
Extract the issue number from the branch name (`<type>/<issue-number>-<kebab>`, see `.agents/skills/git-commit/SKILL.md`).
...
Read `.agents/skills/write-pr/references/labels.md` for the last-known label set and a commit-type → label mapping suggestion, but treat the live `gh label list` output as authoritative. Select 0–2 labels; labeling is a convention, not a hard requirement (past PRs were often unlabeled).
